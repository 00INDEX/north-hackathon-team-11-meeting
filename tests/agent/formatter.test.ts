...
test("formatAgentMessage formats available room list", () => {
  ...
  assert.equal(
    response.reply,
    "2026-08-04 10:00—11:00，筛选条件：小会议室、至少 6 人可用会议室：506 6 人（A 座）；507 8 人（B 座）。",
  );
});
...
test("formatAgentMessage formats booking success", () => {
  ...
  assert.equal(
    response.reply,
    "预约成功：「项目讨论」 2026-08-05 10:00—11:00 506。",
  );
});
...
test("formatAgentMessage formats cancellation result", () => {
  ...
  assert.equal(
    response.reply,
    "已取消预约：「项目讨论」 2026-08-05 10:00—11:00 506。",
  );
});
...
test("formatAgentMessage formats unavailability rule update", () => {
  ...
  assert.equal(
    response.reply,
    "更新不可预约规则成功，已更新规则 rule-1。目标：506，2026-08-05 13:00—18:00，原因：下午维修。",
  );
});
...
test("formatAgentMessage formats conflict reason", () => {
  ...
  assert.equal(
    response.reply,
    "预约冲突：活动室午餐规则（2026-08-05 11:30 至 2026-08-05 13:30），午餐时段不可预约",
  );
});
...
test("formatAgentMessage formats permission, parse, and backend unavailable errors", () => {
  ...
  assert.equal(
    ...
    "抱歉，当前账号没有权限执行这个操作。你可以联系管理员确认权限后重试。",
  );
  ...
  assert.equal(
    ...
    "抱歉，我暂时没有解析成功：LLM parser response is not valid JSON.。请换一种说法重试，或提供更明确的日期、时间、房间和标题。",
  );
...
test("formatAgentMessage never claims success for empty or malformed backend results", () => {
  const cases = [
    {
      intent: {
        type: "create_booking" as const,
    ...
    {
      intent: {
        type: "create_combined_room" as const,
  ...
  for (const { intent, data } of cases) {
    const response = formatAgentMessage({
      intent,
      orchestratorResult: { actions: [], data },
    });
    assert.equal(response.error?.type, "backend_unavailable");
    assert.doesNotMatch(response.reply, /成功|已提交|已创建/);
  }
});
