...
import type { ConversationHistoryEntry, NexParserRequest, NexParserResult } from '../../src/agent/nex.js';
...
test('RFC-0002 scenario: tomorrow noon activity-room booking reports lunch rule conflict', async () => {
  ...
  const client = createScenarioBusinessApiClient({
    createBooking: async (request) => {
      calls.push({ endpoint: '/api/bookings', payload: request });
      throw {
        type: 'conflict',
...
test('RFC-0002 scenario: combined room creation sends component room IDs and reports locked components', async () => {
  ...
  const client = createScenarioBusinessApiClient({
    createCombinedRoom: async (request) => {
      calls.push({ endpoint: '/api/room-groups', payload: request });
      return {
        room: {
          id: 'combined_room_1_2',
          name: '大会议室',
          capacity: 12,
          type: 'combined',
          location: 'A 座',
  ...
  assert.equal(orchestratorResult.error, undefined);
  assert.deepEqual((orchestratorResult.data as { room: { componentRoomIds: string[] } }).room.componentRoomIds, [
    'room-meeting-1',
    'room-meeting-2',
  ]);
  assert.match(reply.reply, /组合会议室 combined_room_1_2 创建成功/);
...
test('RFC-0002 scenario: 506 rule correction updates the same rule instead of creating a new one', async () => {
  ...
  const client = createScenarioBusinessApiClient({
    ...
    updateUnavailabilityRule: async (request) => {
      calls.push({ endpoint: '/api/unavailability-rules/last', payload: request });
      return {
...
test('RFC-0002 scenario: end-to-end Agent API handles normalized 506 rule correction', async () => {
    ...
    body: JSON.stringify({
      ...
      history: [
        ...
        },
        {
          ...
          actions: [{ type: 'create_unavailability_rule', endpoint: '/api/unavailability-rules' }],
        },
      ],
    }),
    headers: { 'content-type': 'application/json' },
  ...
  const createPayload = await createResponse.json() as Awaited<ReturnType<typeof createResponse.json>>;
  const updatePayload = await updateResponse.json() as Awaited<ReturnType<typeof updateResponse.json>>;
  ...
  assert.deepEqual(createPayload.parsedIntent.timeRange, { startTime: '00:00', endTime: '24:00' });
  assert.equal(updatePayload.parsedIntent.type, 'update_last_unavailability_rule');
  assert.deepEqual(updatePayload.parsedIntent.timeRange, { startTime: '13:00', endTime: '18:00' });
...
function createScenarioBusinessApiClient(partialClient: Partial<BackendBusinessApiClient> = {}): BackendBusinessApiClient {
  return {
...
function createScenarioAgentApp(partialClient: Partial<BackendBusinessApiClient> = {}) {
  const app = new Hono();
  ...
  app.route('/', createAgentMessageRoute({
    parser: {
      async parseIntent(request: NexParserRequest): Promise<NexParserResult> {
        const rawIntent = inferIntentFromMessage(request.message, request.history ?? []);
        const normalized = normalizeIntentTimeFields(rawIntent, { today: TODAY });
        if (!normalized.valid) {
          return { error: { type: 'parse_failed' as const, message: normalized.error.message } };
        }
        const parsed = parseAgentIntent(JSON.stringify(normalized.intent));
        if (parsed.error) {
          return { error: { type: 'parse_failed' as const, message: parsed.error.message, details: parsed.error.details } };
        }
...
function inferIntentFromMessage(message: string, history: ConversationHistoryEntry[]): Record<string, unknown> {
  ...
  if (message.includes('刚才说错了')) {
    const lastRuleIntent = [...history].reverse().find((entry) => entry.role === 'assistant' && isRecord(entry.parsedIntent) && entry.parsedIntent.type === 'create_unavailability_rule');
    const lastParsedIntent = lastRuleIntent && isRecord(lastRuleIntent.parsedIntent) ? lastRuleIntent.parsedIntent : undefined;
    const target = lastParsedIntent && isRecord(lastParsedIntent.target) ? 'room-506' : 'room-506';
    const date = lastParsedIntent && isRecord(lastParsedIntent.date) ? '2026-07-29' : '2026-07-29';
    return {
      type: 'update_last_unavailability_rule',
      target,
      date,
