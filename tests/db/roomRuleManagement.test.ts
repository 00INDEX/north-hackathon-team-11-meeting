...
describe("RFC-0001 T2 room configuration and rule management", () => {
  ...
  it("lists rooms with combined resources and seeds baseline system rules", async () => {
    ...
    expect(
      rooms
        .find((room) => room.id === "room-combined")
        ?.resources?.map((resource) => resource.id),
    ).toEqual(["resource-meeting-1", "resource-meeting-2"]);
  ...
  it("creates a combined room through the minimal authoritative API and shares component resources", async () => {
    const createResponse = await app.request("/api/rooms/combined", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-actor": "local-admin",
        "x-idempotency-key": "create-agent-combined",
      },
      body: JSON.stringify({
        id: "room-agent-combined",
        name: "Agent 联合会议室",
    ...
    await expect(createResponse.json()).resolves.toMatchObject({
      ...
      name: "Agent 联合会议室",
      type: "组合会议室",
      resources: [{ id: "resource-meeting-1" }, { id: "resource-meeting-2" }],
    });

    const replayResponse = await app.request("/api/rooms/combined", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-actor": "local-admin",
    ...
    const combinedReservation = reservationService.create({
      id: "reservation-agent-combined",
      title: "组合会议",
      roomId: "room-agent-combined",
      start: combineLocalDateTime("2026-01-07", "09:00"),
      end: combineLocalDateTime("2026-01-07", "10:00"),
    });
    expect(() =>
      reservationService.create({
        id: "reservation-component-conflict",
        title: "组件房间冲突",
        roomId: "room-meeting-1",
        start: combinedReservation.start,
        end: combinedReservation.end,
      }),
  ...
  it("updates room open hours, audits the change, and rejects stale versions", async () => {
    ...
    const auditResponse = await app.request(
      "/api/audit-events?targetType=room&targetId=room-502",
    );
  ...
  it("creates and patches a dynamic rule in place so reads see one current row", async () => {
    ...
    const rulesAfterCreate = (await (
      await app.request("/api/rules")
    ).json()) as Rule[];
    expect(
      rulesAfterCreate.filter((rule) => rule.id === "rule-504-maintenance"),
    ).toHaveLength(1);
    ...
    const rulesAfterPatch = (await (
      await app.request("/api/rules")
    ).json()) as Rule[];
    expect(
      rulesAfterPatch.filter((rule) => rule.id === "rule-504-maintenance"),
    ).toHaveLength(1);

    const staleRuleResponse = await app.request(
      "/api/rules/rule-504-maintenance",
      {
        method: "PATCH",
        headers: { "content-type": "application/json" },
    ...
    const auditResponse = await app.request(
      "/api/audit-events?targetType=rule&targetId=rule-504-maintenance",
    );
    expect(auditResponse.status).toBe(200);
    const auditEvents = (await auditResponse.json()) as Array<{
      eventType: string;
  ...
  it("protects baseline system rules from physical deletion", async () => {
    const deleteResponse = await app.request(
      "/api/rules/rule-502-tuesday-all-day",
      { method: "DELETE" },
    );
    ...
    const rules = (await rulesResponse.json()) as Rule[];
    expect(
      rules.find((rule) => rule.id === "rule-502-tuesday-all-day"),
    ).toMatchObject({
      isSystem: true,
      enabled: true,
    });
