...
describe("SQLite migrations and seed data", () => {
  ...
  beforeEach(() => {
    const tempDir = mkdtempSync(
      path.join(os.tmpdir(), "meeting-room-baseline-"),
    );
  ...
  it("seeds the baseline rooms, combined mapping, rules and persistence repositories idempotently", () => {
    ...
    expect(combined?.resources.map((resource) => resource.id)).toEqual([
      "resource-meeting-1",
      "resource-meeting-2",
    ]);
    ...
    expect(new Set(rules.map((rule) => rule.targetId))).toEqual(
      new Set(["room-activity", "room-502"]),
    );
    expect(rules.every((rule) => rule.isSystem)).toBe(true);
    ...
    expect(
      reservationRepository.resourceSnapshotsForReservation(reservation.id),
    ).toEqual([
    ...
    expect(auditRepository.findById(auditEvent.id)?.eventType).toBe(
      "room.seeded",
    );
