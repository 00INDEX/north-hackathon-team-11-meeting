...
export function formatAgentMessage(
  options: FormatAgentMessageOptions,
  ...
  if (!options.intent) {
    return {
      reply:
        "抱歉，我暂时没有理解你的请求。请把要查询、预约、取消或修改的内容说得再具体一些。",
      parsedIntent: null,
      actions: [],
      error: null,
    };
  ...
  const invalidSuccess = validateSuccessData(
    options.intent,
    orchestratorResult.data,
  );
  if (invalidSuccess) {
    return {
      reply: formatErrorReply(invalidSuccess),
      parsedIntent: options.intent,
      actions: orchestratorResult.actions,
  ...
  return {
    reply: formatOrchestratorSuccessReply(
      options.intent,
      orchestratorResult.data,
...
function formatOrchestratorSuccessReply(
  intent: AgentIntent,
  data: unknown,
  actions: AgentAction[],
): string {
  switch (intent.type) {
    case "query_available_rooms":
      return formatAvailabilityReply(
        data as AvailabilityCheckResult,
        intent.date,
        intent.timeRange,
        intent.filters,
      );
    ...
    case "create_unavailability_rule":
      return formatUnavailabilityRuleReply(
        data as UnavailabilityRuleResult,
        actions,
        "创建",
      );
    case "update_last_unavailability_rule":
...
function formatAvailabilityReply(
  ...
  const rooms = Array.isArray(result?.availableRooms)
...
function formatBookingResultReply(
  ...
  if (!booking) {
    return `${actionLabel}失败：后端没有返回有效预约详情。`;
  }
...
function formatUnavailabilityRuleReply(
  result: UnavailabilityRuleResult,
  actions: AgentAction[],
  actionLabel: "创建" | "更新",
): string {
...
function formatRoomResultReply(data: unknown, actionLabel: string): string {
  ...
  if (!room) {
    return `${actionLabel}会议室配置失败：后端没有返回有效会议室详情。`;
  }

  return `${actionLabel}会议室配置成功：${formatRoom(room)}。`;
}

function formatCombinedRoomReply(
  data: unknown,
  intent: Extract<AgentIntent, { type: "create_combined_room" }>,
): string {
  const room = extractRoom(data);
  const componentText = intent.componentRoomIds.join("、");

  if (room) {
    return `组合会议室 ${room.id} 创建成功：${formatRoom(room)}，组件房间：${componentText}。`;
  }

  return `组合会议室 ${intent.combinedRoomId} 创建失败：后端没有返回有效会议室详情。`;
}
...
function validateSuccessData(
  ...
  switch (intent.type) {
    case "query_available_rooms":
      valid =
        ...
        data.availableRooms.every(isRoom);
      break;
    case "create_booking":
    case "cancel_booking":
      valid =
      ...
      break;
    case "create_unavailability_rule":
      valid = isRecord(data) && isRule(data.rule);
      break;
    case "update_last_unavailability_rule":
      valid =
        isRecord(data) &&
        typeof data.updatedRuleId === "string" &&
        data.updatedRuleId.trim() !== "" &&
        (data.rule === undefined || isRule(data.rule));
      break;
    case "create_or_update_room":
    case "create_combined_room":
      valid = Boolean(extractRoom(data));
      break;
    case "need_clarification":
      return undefined;
    default:
  ...
  return valid
    ...
    : {
        ...
        message: "Backend API returned an invalid success response.",
      };
}
...
function formatConflictErrorReply(error: AgentError): string {
  ...
  const summary =
    conflicts.length > 0 ? formatConflictSummary(conflicts) : error.message;
  return `预约冲突：${summary}`;
}
  ...
  const parts = conflicts.map((conflict) => {
    ...
    const scope =
      conflict.start && conflict.end
        ? `（${formatDateTime(conflict.start)} 至 ${formatDateTime(conflict.end)}）`
        : "";
...
function formatRuleScope(
  ...
  const target = rule.target
    ? formatTargetName(rule.target)
    : fallbackTarget
      ? formatTargetName(fallbackTarget)
      : "目标";
...
function isRule(value: unknown): value is UnavailabilityRule {
  return (
    isRecord(value) &&
    typeof value.id === "string" &&
    value.id.trim() !== "" &&
...
function extractRoom(value: unknown): Room | undefined {
  if (isRoom(value)) {
    return value;
  }
  return isRecord(value) && isRoom(value.room) ? value.room : undefined;
}
