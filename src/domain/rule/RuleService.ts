...
import type { AvailabilityRule, CreateAvailabilityRuleInput, UpdateAvailabilityRuleInput } from './types';
...
export class RuleService {
  ...
  constructor(
    ...
    private readonly roomRepository = new RoomRepository(db),
    private readonly resourceRepository = new ResourceRepository(db),
  ...
  create(input: CreateAvailabilityRuleInput, context: AuditContext = {}): AvailabilityRule {
  ...
  update(id: string, patch: UpdateAvailabilityRuleInput, context: AuditContext = {}): AvailabilityRule {
    const existing = this.repository.findById(id);
    if (!existing) {
      throw new AppError('NOT_FOUND', `规则不存在: ${id}`, { conflicts: [{ type: 'rule', id, name: id }] });
    }

    if (patch.version !== undefined && patch.version !== existing.version) {
      throw new AppError('VERSION_CONFLICT', `规则版本已过期: ${id}`, {
        conflicts: [{ type: 'version', id, name: existing.reason, reason: `当前版本为 ${existing.version}` }],
      });
    }
  ...
  delete(id: string, context: AuditContext = {}): AvailabilityRule {
    ...
    if (existing.isSystem) {
      throw new AppError('VALIDATION_ERROR', '业务基线规则不可删除，只能调整范围或停用动态规则');
    }

    const updated = this.repository.upsert({ ...existing, enabled: false, version: existing.version + 1 });
    this.auditService.record(
      {
        id: `audit-${randomUUID()}`,
...
function validateRuleInput(rule: AvailabilityRule, service: RuleService): void {
  ...
  if (!['room', 'resource'].includes(rule.targetType)) {
    throw new AppError('VALIDATION_ERROR', `不支持的规则目标类型: ${rule.targetType}`);
  }
  if (!['open_hours', 'periodic_block', 'one_time_block'].includes(rule.ruleType)) {
    throw new AppError('VALIDATION_ERROR', `不支持的规则类型: ${rule.ruleType}`);
  }
...
function validateUtcInterval(start: string, end: string): void {
  ...
  if (!startDate || !endDate) {
    throw new AppError('VALIDATION_ERROR', '规则起止时间必须是合法 UTC ISO 时间');
  }

  if (startDate.getTime() >= endDate.getTime()) {
    throw new AppError('VALIDATION_ERROR', `规则起止时间必须满足 start < end: ${start}–${end}`);
  }
}
...
function validateRuleClockRange(start: string, end: string): void {
  ...
  if (startTime >= endTime) {
    throw new AppError('VALIDATION_ERROR', `周期规则时间必须满足 start < end: ${start}–${end}`);
  }
}
...
function validateTarget(
  ...
  if (!found) {
    throw new AppError('NOT_FOUND', `规则目标不存在: ${targetType}:${targetId}`);
  }
}
