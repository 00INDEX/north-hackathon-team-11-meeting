...
import type { CreateAvailabilityRuleInput, UpdateAvailabilityRuleInput } from '@/domain/rule/types';
...
export function createRuleRoutes(db: Database) {
  ...
  app.get('/:ruleId', (c) => {
    ...
    if (!rule) {
      return c.json({ error: { code: 'NOT_FOUND', message: `规则不存在: ${c.req.param('ruleId')}` } }, 404);
    }
    return c.json(rule);
  });
  ...
  app.patch('/:ruleId', async (c) => {
    ...
    return c.json(service.update(c.req.param('ruleId'), patch, auditContext(c)));
  });

  app.delete('/:ruleId', (c) => c.json(service.delete(c.req.param('ruleId'), auditContext(c))));
