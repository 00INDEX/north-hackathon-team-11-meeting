...
export function runMigrations(db: Database): void {
  ...
  const existing = db.prepare('SELECT id FROM schema_migrations').all() as Array<{ id: number }>;
...
export function getMigrations(db: Database): MigrationRecord[] {
  return db.prepare('SELECT id, name, applied_at AS appliedAt FROM schema_migrations ORDER BY id').all() as MigrationRecord[];
}
