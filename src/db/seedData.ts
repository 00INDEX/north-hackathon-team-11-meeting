...
function existingTimestamps(
  ...
  const row = db
    .prepare(
      `SELECT created_at AS "createdAt", updated_at AS "updatedAt" FROM ${table} WHERE id = ?`,
    )
    .get(id) as ExistingTimestamps | undefined;
  return row ?? { id };
}
...
export function seedBaselineRooms(db: Database): void {
  ...
  const resources: CreateResourceInput[] = [
    ...
    {
      id: "resource-meeting-1",
      name: "会议室一",
      type: "standard-meeting-room",
    },
    {
      id: "resource-meeting-2",
      name: "会议室二",
      type: "standard-meeting-room",
    },
  ...
  db.transaction(() => {
    roomRepository.upsertResources(
      resources.map((resource) =>
        withExistingTimestamps(db, "resources", resource),
      ),
    );
    ...
    roomRepository.replaceRoomResources("room-meeting-1", [
      "resource-meeting-1",
    ]);
    roomRepository.replaceRoomResources("room-meeting-2", [
      "resource-meeting-2",
    ]);
    roomRepository.replaceRoomResources("room-combined", [
      "resource-meeting-1",
      "resource-meeting-2",
    ]);
