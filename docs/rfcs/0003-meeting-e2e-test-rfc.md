...
本 RFC 与现有设计 RFC 的关系如下：

- `RFC-0001-local` 是前后端业务脚手架的事实来源，定义房间、规则、时间、冲突、组合空间、事务和强制调整等核心业务事实；本 RFC 只负责验收这些能力是否按 Demo 场景正确连通。
- `RFC-0002` 是 Agent 编排的事实来源，定义 `AgentIntent`、LLM Parser、Orchestrator、后端 API 和响应契约；本 RFC 只负责验证 Agent 是否按这些契约调用后端，并断言最终状态。
- 本 RFC 不重新定义业务实现或 Agent 实现；如果发现 0001/0002 与测试结论冲突，应优先修订对应 RFC 或实现，而不是在测试 RFC 中隐式绕过。

当前仓库已有 `TEST_PLAN.md` 和测试目录初稿。本 RFC 将测试工作提升为正式设计约束：Agent 层必须可观测，规则层必须可脱离模型独立验证，拒绝类场景必须同时断言回复与数据库状态，Demo 必须围绕赛题基础场景与挑战功能形成闭环。
...
本 RFC 特别强调跨 RFC 对齐：业务事实以 0001 为准，Agent 契约以 0002 为准，测试只验证二者组合后的端到端行为。
...
| 层 | 对象 | 断言方式 | 通过标准 | 当前规模 |
|---|---|---|---|---|
| L1 规则引擎 | service / 纯函数 / 后端规则逻辑 | 精确值比对；业务事实以 `RFC-0001-local` 为准 | 100% 必过 | 42 条 |
| L2 API / Tools | HTTP 接口、Agent 可调工具 | 状态码 + DB 状态 | 100% 必过 | 18 条 |
| L3 Agent 语义 | 自然语言 → 工具调用 → 状态；Agent 契约以 `RFC-0002` 为准 | 工具调用参数 + DB 状态 + rubric | 采样 3–5 次，≥2 次过 | 37 条 |
...
```mermaid
flowchart TD
    A[确认 0001/0002 业务与 Agent 口径] --> B[L1 规则引擎测试]
    B --> C[L2 API / Tools 测试]
    C --> D[L3 Agent 语义测试]
    D --> E[L4 Demo 主线]
    E --> F[L5 非功能与现场降级检查]
...
- **Room / 空间**：可预约的物理空间或组合空间。物理空间包括活动室、会议室一、会议室二、502、503、504、505、506；组合空间 `combined_1_2` 代表会议室一和会议室二合并后的大会议室，但不是第三个独立物理房间。
- **Resource / 物理资源**：发生排他占用的实际空间。组合空间预约必须映射到会议室一和会议室二两个物理资源。
- **Booking / 预约**：成员或管理员创建的占用记录，必须进入系统状态并参与冲突校验。
- **DisableRule / 禁用规则**：周期性或一次性不可预约规则，包括活动室工作日午餐用途、502 每周二全天不可用、504 临时维修、活动禁用等。
- **Agent Tool / Agent 工具**：NAC Agent 可调用的确定性函数或后端 API。所有写操作必须收敛到工具/API，Agent 只负责解析自然语言并填写参数。
- **Test Adapter / 测试适配器**：测试脚本中封装的后端、Agent、数据库状态查询接口。测试脚本不直接绕过 adapter 依赖具体表结构。
...
NAC Agent/Agent Orchestrator 应暴露可观测的工具调用或后端 API 调用。测试应要求工具调用日志或结构化返回中包含：
...
组合空间 `combined_1_2` 是用户可见的可预订空间，但冲突校验发生在底层物理资源上。必须验证：

- 订 `combined_1_2` 前，会议室一和会议室二都必须可用；
- 订 `combined_1_2` 后，会议室一和会议室二同时间段不可再单独预约；
- 会议室一或会议室二已预约时，不能再订 `combined_1_2`；
- 取消 `combined_1_2` 后，会议室一和会议室二同时释放；
- 日历或平面图必须在一、二两行都显示组合占用，并标注来源为组合空间。

#### 决策 5：多轮修正必须 update 而不是 insert

赛题明确包含：“这周三 504 临时维修，全天不能预约。刚才说错了，只停用下午。”结合 0001 的业务事实，504 是已存在的小会议室，测试应验证：

- 第一条消息创建一条 504 周三全天维修规则；
- 第二条消息解析为更新上一条规则，而不是插入新规则；
- 更新后规则表仍只有 1 条 504 维修规则；
- 该规则时段变为下午，上午恢复可用；
- 日历/平面图状态同步变化。
...
本 RFC 的接口契约只规定测试需要观察的 Agent 工具/API 行为和状态结果；具体 API 路径、类型和错误码以 `RFC-0001-local` 与 `RFC-0002` 为准。

#### 后端测试适配接口

测试 adapter 当前假设以下方法存在：
...
| ID | 名称 | 类型 | 容量 | 设备 | 固定规则 |
|---|---|---|---|---|---|
| `activity` | 活动室 | 物理 | 40 | 投影、音响 | 工作日 11:30–13:30 用作餐厅 |
| `room1` | 会议室一 | 物理 | 12 | 显示屏、白板、视频会议 | 可与 room2 组合 |
| `room2` | 会议室二 | 物理 | 12 | 显示屏、白板、视频会议 | 可与 room1 组合 |
| `combined_1_2` | 大会议室 | 组合空间 | 24 | 投影、白板、视频会议 | 占用时同步占用会议室一/会议室二 |
| `502` | 502 | 物理 | 6 | 显示屏、白板 | 每周二全天不可用 |
| `503` | 503 | 物理 | 6 | 显示屏、白板 | 无固定禁用 |
| `504` | 504 | 物理 | 6 | 显示屏、白板 | 无固定禁用；用于临时维修场景 |
| `505` | 505 | 物理 | 6 | 显示屏、白板 | 无固定禁用 |
| `506` | 506 | 物理 | 6 | 显示屏、白板 | 无固定禁用 |

说明：组合空间是用户可见的可预订空间，但冲突校验必须映射到会议室一、会议室二两个物理资源；测试用例不应把组合空间当作第三间独立物理房间。
...
范围：基于 `TEST_PLAN.md` 和现有测试目录完善 L1-L5 用例；如保留机器可读用例文件，应同步修正房间与规则口径。  
验收标准：

- L1 覆盖重叠、周期性禁用、午餐用途、组合空间、动态禁用、容量/设备、未知房间；
...
- 覆盖小会议室查询、活动室午餐拒绝、组合空间、冲突、动态禁用修正、admin 取消、平面图；
- 支持 reset 后重跑。
...
- 502 每周二不可用，且不误伤 503/504/505/506 和周三；
...
- AG-001：下周二小会议室查询，结果含 503/504/505/506 中未占用小会议室，不含 502，且不得直接下单；
- AG-002：明天中午活动室预约被午餐规则拒绝，DB 无新增；
- AG-003：本周五合并会议室一和二，产生 1 条组合预约，会议室一、二同时间段不可用；
- AG-004：504 多轮动态禁用修正，规则表只更新 1 条；
...
1. 列表视图展示活动室、会议室一/二、组合空间和 502–506；
2. 查询下周二 10:00–11:00 小会议室，只出不含 502 的可用小会议室；
3. 预订 503，日历出现色块；
4. 另一用户订 503 同时段，出现冲突提示与替代方案；
5. 明天中午预约活动室，被午餐规则拒绝；
6. 本周五 14:00–16:00 合并会议室一和二；
7. 查询会议室一周五下午，说明被组合预约占用；
8. 管理员创建周三 504 全天维修规则；
...
1. **相对时间 mock 能力**：是否已实现 `X-Mock-Now` 请求头或 `MOCK_NOW` 环境变量，决定 L3/L4 是否可稳定复现。
2. **权限模型**：0001 首版无真实鉴权，但 0002 要求后端最终校验 admin/member 权限；测试账号和角色注入方式仍需确认。
3. **活动室午餐规则是否仅工作日生效**：0001 的业务基线写的是工作日 11:30–13:30；若团队决定午餐规则不区分工作日，应明确说明。
4. **禁用规则与已有预约冲突的 force 策略**：0001 已设计 `force=true` 原子取消冲突预约并记录原因；测试需确认是否覆盖已有预约冲突场景。
5. **地图/平面图视图范围**：0001 首版明确不包含地图/平面图，但赛题挑战功能包含该能力；需要确认它作为 Demo 验收项还是后续增强项。

## 参考资料

- Hackathon Topic A：会务系统赛题说明
- `TEST_PLAN.md`
- `docs/rfcs/0001-local-meeting-room-system.md`
- `docs/rfcs/0002-meeting-room-agent-orchestrator.md`
