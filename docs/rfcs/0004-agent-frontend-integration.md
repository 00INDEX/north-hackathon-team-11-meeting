...
- [x] Phase 1：把 Agent 业务调用收敛到 RFC-0001 权威 API，并明确旧 facade 废弃边界。
- [x] Phase 2：实现服务端会话历史与 Demo 身份注入，调整 Agent API 请求边界。
- [x] Phase 3：接入真实 Nex parser 默认调用与运行门禁。
- [x] Phase 4：在现有页面新增 Agent 聊天面板并联通 `POST /api/agent/message`。
- [x] Phase 5：完成查询、预约、取消、规则管理的端到端验收与文档。
...
| ID  | 标题                                      | 依赖   | Ref |
| --- | ----------------------------------------- | ------ | --- |
| T1  | 权威后端 API 适配与旧业务 facade 废弃边界 | -      | `d80ec5f` |
| T2  | 服务端会话历史与 Demo 身份注入            | T1     | `d80ec5f` |
| T3  | 真实 Nex 联调配置、默认调用与运行门禁     | T1, T2 | `d80ec5f` |
| T4  | 现有页面新增 Agent 聊天面板               | T2, T3 | `d80ec5f` |
| T5  | 查询、预约、取消、规则管理端到端验收      | T4     | `d80ec5f` |
| T6  | 文档、兼容说明与 RFC 状态收尾             | T5     | `d80ec5f` |
